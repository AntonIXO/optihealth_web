...
import { numberColorToRGBA } from '../helpers';

export default function makePercentageArea(ctx: CanvasRenderingContext2D, linesData: any, cache: any = makePercentageAreaCache(linesData)) {
...
export function makePercentageAreaCache(linesData: any) {
  const linesKeys = Object.keys(linesData);
  const dataLength = linesData[linesKeys[0]].values.length;
  const linesCache: any = {};
