...
import { mixNumbers, formatNumberWithThousandGroups } from '../helpers';
import { roundedRectanglePath } from '../helpers';
import { getDayInWeekAndMonth, months, timestampInDay } from '../helpers';
import { memoizeObjectArguments } from '../helpers';
import { TYPE_AREA, TYPE_BAR } from '../constants';
import {
  fontFamily,
  textColors,
...
export default function makeColumnDetails(ctx: CanvasRenderingContext2D, type: string, linesData: any) {
  ...
  const getColumnsSums = hasTotalRow || hasStakes ? makeColumnsSums(linesData) : () => ({ columnsSums: [], linesInSumCount: 0 });

  const linesValueTextGetters: any = {};
  for (let i = 0; i < linesKeys.length; ++i) {
    const key = linesKeys[i];
    linesValueTextGetters[key] = (index: number) => {
  ...
  function drawBackground(pixelRatio: number, backgroundColor: number, shadowColor: number, shadowOpacity: number, opacity: number, x: number, y: number, width: number, height: number) {
  ...
  function drawHeader(pixelRatio: number, textColor: number, day: number, month: number, year: number, opacity: number, x: number, y: number) {
  ...
  function drawRow(key: string, index: number, x: number, y: number, width: number, textColor: number, opacity: number, pixelRatio: number, linesOpacity: any) {
    ...
    if (hasStakes) {
      const {columnsSums, linesInSumCount} = getColumnsSums(linesOpacity) as any;
    ...
    drawRotatingDisplay({
      ...
      getItemText: isTotalRow
        ? (index: number) => formatNumberWithThousandGroups((getColumnsSums(linesOpacity) as any).columnsSums[index])
...
function getLineRowsAmount(linesData: any, linesOpacity: any) {
  let amount = 0;
...
function doesHaveTotalRow(linesData: any, type: string) {
  return type === TYPE_BAR && Object.keys(linesData).length > 1;
}

function doesHaveStakes(type: string) {
