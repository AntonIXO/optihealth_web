...
function Calendar({
  ...
  const mergedClassNames: typeof defaultClassNames = (() => {
    const acc = {} as typeof defaultClassNames;
    for (const k of Object.keys(defaultClassNames)) {
      const key = k as keyof typeof defaultClassNames;
      const override = classNames?.[key as keyof typeof classNames];
      acc[key] = override
        ? cn(defaultClassNames[key], override as string)
  ...
  const defaultComponents = {
    Chevron: (
      ...
      const common = { size: props.size ?? 16, strokeWidth: 2, "aria-hidden": true } as const;
